...
Перем ЧислоПопытокОтправкиСообщения Экспорт;
...
//    * Вложения                            - Строка или СписокЗначений, состоящий из строк - Путь 
//			к вложениями сообщения
//
Процедура ИнициироватьПараметры()
	...
	// Способ Аутентификации SMTP
	//  0 - Без аутентифиации
	//  1 - Использовать базовую (открытым текстом) проверку подлинности.
	//  2 - Использовать проверку подлинности ntlm
	УчетнаяЗаписьЭП.Вставить("СпособАутентификацииSMTP", 2);
	// Таймаут
	УчетнаяЗаписьЭП.Вставить("Таймаут", 60);
	...
	// ЧИСЛО ПОПЫТОК
	ЧислоПопытокОтправкиСообщения = 5;
...
Функция ОтправитьСообщение() Экспорт
	...
	loConfig.Fields.Item("http://schemas.microsoft.com/cdo/configuration/smtpauthenticate").Value = 
		УчетнаяЗаписьЭП.СпособАутентификацииSMTP;
	// Использование протокола SSL
	//  0 - Не использовать
	//  1 - Использовать
	loConfig.Fields.Item("http://schemas.microsoft.com/cdo/configuration/smtpusessl").Value = 
		?(УчетнаяЗаписьЭП.ИспользоватьSSL, 1, 0);
	// Число секунд ожидания для службы SMTP
	loConfig.Fields.Item("http://schemas.microsoft.com/cdo/configuration/smtpconnectiontimeout").Value = 
		УчетнаяЗаписьЭП.Таймаут;
	// Обновляем настройки
	loConfig.Fields.Update();
	
	// Отправка сообщения
	Для СчЧислоПопыток = 1 По ЧислоПопытокОтправкиСообщения Цикл
		Попытка
			loCdoMessage.Send();
			ТекстОшибки = "";
			Возврат Истина;
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			Если СчЧислоПопыток < ЧислоПопытокОтправкиСообщения Тогда
				Приостановить(10000 * СчЧислоПопыток);
			КонецЕсли;
		КонецПопытки;
	КонецЦикла;
